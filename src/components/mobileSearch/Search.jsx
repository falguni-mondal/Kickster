...
const Search = () => {
    ...
    useEffect(() => {
        if (product) {
            if (searchTxt !== "") {
                const filteredProducts = product.filter(p => p.title.startsWith(searchTxt) || p.brand.startsWith(searchTxt));
        ...
        <div className='w-full xl:hidden h-screen px-5 py-3'>
            <div className="mob-searchbar relative w-full">
                ...
                <div className="result absolute top-[50px] left-0 bg-white z-[2] p-2 rounded-md overflow-y-auto max-h-[300px] w-full flex flex-col gap-2">
                    {
                        search.map(product => (
                            <NavLink to={`/product/${product.id}`} className="search-item flex items-center gap-2">
            ...
            <section className='recommendation-container w-full mt-10'>
                ...
                <div className="recommendations grid grid-cols-5 grid-rows-6 gap-1 h-[450px]">
                    <div className='h-full flex justify-center items-center col-span-2 row-span-2 rounded-md overflow-hidden relative'>
                    ...
                    <div className='h-full flex justify-center items-center col-span-3 row-span-2 rounded-md overflow-hidden relative'>
                        <span className='text-white txt-medium'>
                            Women
                        </span>
                    ...
                    <div className='h-full flex justify-center items-center col-span-5 row-span-2 rounded-md overflow-hidden relative'>
                        <span className='text-white txt-medium'>
                            Brands
                        </span>
